...
import matplotlib.pyplot as plt # Se mantiene por si hay uso futuro
import base64
from io import BytesIO
from fpdf import FPDF
from docx import Document

# --- Configuración de la Página (¡Novedad!) ---
st.set_page_config(
    page_title="App Hidrometeorológica Avanzada",
    page_icon="💧",
    layout="wide", # ¡Sorpresa: La aplicación ocupa todo el ancho!
...
def login():
    ...
    with col_login_img:
        st.image("https://images.unsplash.com/photo-1549740449-74e2d3b2c6a2?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                 caption="Monitoreo Inteligente del Clima", use_column_width=True)
...
def logout():
    ...
    st.session_state.df_cargado = None # Limpiar el DF cargado al cerrar sesión
    st.rerun()
...
def generar_pdf(df_to_export):
    ...
    # Añadir un resumen básico del DF al PDF
    ...
    # Convertir columnas a string para evitar errores con tipos mixtos
    df_str = df_to_export.astype(str)

    # Calcular ancho de columnas dinámicamente
    # Ajustar para incluir el índice si es parte de la tabla
    num_cols_pdf = len(df_str.columns)
    if df_str.index.name is not None:
        num_cols_pdf += 1

    col_width = pdf.w / (num_cols_pdf + 1) # Un poco de margen extra
    ...
    # Filas de datos
    pdf.set_font("Arial", '', 8) # Letra más pequeña para los datos
...
def generar_word(df_to_export):
    ...
    table = doc.add_table(rows=1, cols=len(df_to_export.columns) + (1 if df_to_export.index.name else 0)) # +1 si hay nombre de índice
...
def admin_panel():
    ...
    # --- Secciones con Pestañas (¡Sorpresa de Organización!) ---
        ...
        if uploaded_file is None:
            ...
            with st.spinner("Cargando y procesando datos..."):
                try:
                    ...
                    # Lógica para detección de fecha (mejorada)
                    ...
                    if fecha_col_candidatas:
                        for col_name in fecha_col_candidatas:
                            try:
                                ...
                                if not df_copy[col_name].isna().all(): # Si al menos un valor se convirtió
                                    df_copy.set_index(col_name, inplace=True)
                                    st.info(f"Columna '{col_name}' detectada y establecida como índice de tiempo. ✅")
                                    break # Salir si se encontró una columna de fecha válida
                            except Exception:
                                pass # No hacer nada si no se puede convertir
                    ...
                    if not isinstance(df_copy.index, pd.DatetimeIndex):
                         ...
                         if st.checkbox("¿Tu archivo tiene una columna de fecha/hora para el índice?"):
                            date_column_options = [col for col in df.columns if df[col].dtype == 'object'] # Ofrecer columnas de texto
                    ...
                    st.dataframe(df_copy) # Mostrar el DataFrame con el índice ajustado
    ...
    with tab_analisis:
        ...
        if df_actual is not None and not df_actual.empty:
            ...
            # Asegurar que el índice de tiempo esté disponible si es relevante
            if isinstance(df_actual.index, pd.DatetimeIndex):
                index_name = df_actual.index.name if df_actual.index.name else 'Fecha/Índice'
                numeric_cols.insert(0, index_name) # Añadir el índice como opción para ejes numéricos si es fecha
            
            # --- Lógica de Gráficos Dinámicos ---
            if graph_type == "Correlación":
                with col_x_axis: # Ocupa el espacio pero no se usa
                    st.empty()
                with col_y_axis: # Ocupa el espacio pero no se usa
                    st.empty()
                if not numeric_cols:
                    ...
                    try:
                        # Excluir el índice de fecha si se incluyó en numeric_cols para la correlación directa
                        cols_for_corr = [col for col in numeric_cols if col != index_name if isinstance(df_actual[col].dtype, (pd.api.types.is_numeric_dtype))]
                        if cols_for_corr:
                ...
                # Configuración de ejes para los gráficos interactivos
                with col_x_axis:
                ...
                if x_axis and y_axis:
                    try:
                        # Preparar df para Plotly, incluyendo el índice como columna si se selecciona
